...
import com.abc.tagless_final.authn.domain.{AuthnError, RegistrationError}
import com.abc.tagless_final.shapelessext._
...
trait Dsl[F[_]] {
  ...
  def authn(email: String @@ EmailAddress, password: String): F[Either[AuthnError, User]]
...
object Dsl {
  ...
  implicit object StateInterpreter extends Dsl[UserRepositoryState] {
    ...
    override def authn(email: String @@ EmailAddress, password: String): UserRepositoryState[Either[AuthnError, User]] =
      State.inspect(_
